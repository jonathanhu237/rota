...
- **AND** roughly 60% of qualified slots have an `availability_submissions` row per employee
...
- **AND** 1 template named "Realistic Rota" is inserted with 35 `template_slots` covering 5 time blocks × 7 weekdays
- **AND** 70 `template_slot_positions` are inserted: each daytime slot (09:00-10:00, 10:00-12:00, 13:30-16:10, 16:10-18:00) requires `{前台负责人 × 1, 前台助理 × 2}` and the evening slot (19:00-21:00) requires `{外勤负责人 × 1, 外勤助理 × 1}`
- **AND** exactly one publication exists in effective state `ASSIGNING`
- **AND** `availability_submissions` rows are inserted from the embedded weekday vectors, dropping any submission whose slot domain does not match the employee's archetype domain
- **AND** zero assignments exist (so the developer can immediately invoke auto-assign)
- **AND** re-running the scenario yields the same logical scenario shape (same anonymized roster, archetypes, slots, position requirements, and availability vectors), excluding runtime-generated timestamps and bcrypt salts
