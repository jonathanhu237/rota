...
Rota's scheduling, availability, shift coverage, leave and attendance workflows
run on an independent Temvia-generated Go API (`api/`) and React app (`admin/`).
The app uses Vite in development and Caddy in Compose.

## Employee and administrator access

Create an Employee role in **Roles**, select **Employee self-service**
(`rota.self`), and use that role for employee invitations. Employees can submit
their own qualified availability and use the roster, leave, shift-change and
leader-attendance workflows; ownership, qualification and publication windows
are still enforced. This role grants no administrative reads or writes.

`rota.read` grants management read views; `rota.manage` grants scheduling
management. Super Admin retains all capabilities. Temvia requires nonempty
roles, so do not create a permissionless employee role. Existing valid roles
retain their authenticated self-service access.

Migration of an existing Rota database is deferred. Use a new database for this
version; do not point these migrations at the old schema or overwrite an
existing `.env`. The fixed old implementation remains in Git history.

For a disposable current-source business browser check on Centaurus, install
admin dependencies and Chromium, then run `bash scripts/test-business-browser.sh`
under mise Node 24. It builds its own Compose project, tests role boundaries,
availability, localized email links, branding and account switching, and removes
only its own disposable containers and volume.
...
```sh
# New installation only; inspect and adapt an existing .env instead of overwriting it.
...
plus an optional Organization name; both are snapshotted into notification
emails. The System name appears in the interface, browser title, and system
emails regardless of interface or email language. Keep `EMAIL_SETTINGS_ENCRYPTION_KEY`
...
Compose gateway only listens on loopback HTTP. Keep API and database ports
...
For admin development, stop the Compose admin (`docker compose stop admin`)
