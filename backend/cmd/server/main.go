...
func main() {
	...
	mux.HandleFunc("POST /auth/confirm-email-change", authHandler.ConfirmEmailChange)
	...
	mux.HandleFunc("POST /users/me/email-change-request", authHandler.RequireAuth(userHandler.RequestEmailChange))
	...
	mux.HandleFunc("GET /publications/{id}/availability-board", authHandler.RequireAdmin(publicationHandler.ListAdminAvailability))
	mux.HandleFunc("GET /publications/{id}/availability-submissions/{user_id}", authHandler.RequireAdmin(publicationHandler.GetAdminAvailabilityDetail))
	mux.HandleFunc("PUT /publications/{id}/availability-submissions/{user_id}", authHandler.RequireAdmin(publicationHandler.ReplaceAdminAvailability))
