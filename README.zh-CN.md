...
Rota 的排班、空闲时间、代班、请假与考勤功能已接入 Temvia 生成的独立
Go API（`api/`）和 React 应用（`admin/`）。开发使用 Vite，Compose 栈使用 Caddy。

## 员工与管理员权限

在「角色」中创建员工角色，选择「员工自助」（`rota.self`），然后将该角色用于
员工邀请。员工可提交本人有资格班次的空闲时间，并使用班表、请假、换班和负责人
考勤功能；归属、资格和发布周期规则仍然生效。该角色不授予管理读取或写入权限。

`rota.read` 提供管理视图读取，`rota.manage` 提供排班管理能力；超级管理员保留
全部能力。Temvia 要求角色至少有一个权限，不要创建空权限员工角色。已有有效角色
保留原有认证后的自助访问。

旧数据库的数据迁移另行处理。此版本应使用新数据库，不要直接向旧库执行这些迁移，
也不要覆盖现有 `.env`；旧实现保留在 Git 历史中。

在 Centaurus 安装管理端依赖和 Chromium 后，可通过 mise Node 24 运行
`bash scripts/test-business-browser.sh`。脚本创建独立 Compose 测试项目，验证权限、
空闲时间、双语邮件链接、组织品牌及账户切换，结束后只清理自己的临时容器和数据卷。
