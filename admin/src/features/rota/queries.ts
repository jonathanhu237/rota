...
/**
 * Rota's legacy view model is adapted from the canonical auth query. Keeping
 * the shared query key means login, shell guards, and Rota pages observe one
 * current-user record and one cache lifecycle.
 */
export function toRotaUser(user: Awaited<ReturnType<ApiClient["me"]>>): User {
  return {
      id: user.id,
      email: user.email,
      name: user.name,
      is_admin:
        ...
        user.permissions?.includes("rota.read") === true ||
        user.permissions?.includes("rota.manage") === true,
      can_manage:
        user.superAdmin === true || user.permissions?.includes("rota.manage") === true,
      status: "active",
      version: 1,
      language_preference: user.locale?.startsWith("zh") ? "zh" : "en",
...
export const userPositionsQueryOptions = (userID: string) =>
  queryOptions({
...
export const adminAvailabilityDetailQueryOptions = (
  ...
  queryOptions({
    ...
    enabled: publicationID > 0 && userID.trim() !== "",
  })
