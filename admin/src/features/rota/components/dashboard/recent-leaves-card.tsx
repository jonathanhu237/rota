...
import type { Leave, LeaveState } from "@/features/rota/types"
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Link to="/rota/leaves" className={buttonVariants({ variant: "outline" })}>
      ...
      <CardContent>
        {leaves.length > 0 ? (
          ...
          <div className="grid gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
            ...
            <div>
              <Link to="/rota/leaves/new" className={buttonVariants()}>
                {t("leaves.requestCta")}
                <ChevronRight data-icon="inline-end" />
              </Link>
            </div>
