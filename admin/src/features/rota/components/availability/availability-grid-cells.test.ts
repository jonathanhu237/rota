...
import type { QualifiedShift } from "@/features/rota/types"
...
describe("pivotAvailabilityIntoGridCells", () => {
  it("collects distinct time blocks sorted ascending by start and end time", () => {
    ...
    expect(
      result.timeBlocks.map((block) => `${block.start_time}-${block.end_time}`),
    ).toEqual(["09:00-10:00", "09:00-11:00", "12:00-14:00"])
