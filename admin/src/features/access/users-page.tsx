...
export function UsersPage({ api, canManage, actorPermissions, actorSuperAdmin = false, actorID }: { api: ApiClient; canManage: boolean; actorPermissions?: string[]; actorSuperAdmin?: boolean; actorID?: string }) {
  ...
  const canManageRota = canManage && (actorSuperAdmin || actorPermissions === undefined || actorPermissions.includes('rota.manage'))
  ...
  const [qualificationUser, setQualificationUser] = useState<AccessUser | undefined>()
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuGroup>
              ...
              {canManageRota ? <DropdownMenuItem onSelect={() => setQualificationUser(row.original)}><Pencil aria-hidden="true" />{t('common:users.qualifications.manage' as never)}</DropdownMenuItem> : null}
  ...
  ], [canManage, canManageRota, canAssign, actorID, users.isError, i18n.language, lifecycle.isPending, roleAdministrationForbidden, t])
  ...
  return <section className="flex flex-col gap-5" aria-labelledby="users-title">
    ...
    {canManageRota ? <UserPositionsDialog user={qualificationUser} open={qualificationUser !== undefined} canManage={canManageRota} onClose={() => setQualificationUser(undefined)} /> : null}
  </section>
}
