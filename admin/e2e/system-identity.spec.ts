...
async function setLocale(page: Page, locale: 'en' | 'zh-CN'): Promise<void> {
  ...
  if (['/login', '/forgot-password', '/reset-password', '/accept-invitation'].includes(pathname)) {
  ...
  const response = await page.evaluate(async (nextLocale) => {
    const result = await fetch('/api/auth/me/preferences', {
      method: 'PUT',
  ...
  if (!response.ok) throw new Error(`Account locale update failed: ${response.status}`)
}
...
async function saveIdentity(page: Page, systemName: string, expectedStatus = 200): Promise<void> {
  ...
  const responsePromise = page.waitForResponse((response) => response.request().method() === 'PUT' && new URL(response.url()).pathname === '/api/settings/system-identity')
  await section.getByRole('button', { name: 'Save', exact: true }).click()
  const response = await responsePromise
  expect(response.status()).toBe(expectedStatus)
  if (expectedStatus === 200) await expect(section.getByRole('textbox', { name: 'System name', exact: true })).toHaveValue(systemName.trim())
}
...
test.describe('system identity', () => {
  test('publishes identity across public pages, sessions, mail-facing entry points, and settings', async ({ page, browser }, testInfo) => {
    ...
    try {
      ...
      await saveIdentity(conflictB, 'Conflict B', 409)
