...
func SolveAutoAssignments(
	...
	slotCellsByUser := make(map[string]map[slotCellKey]struct{})
	slotPositionKeysByUserCell := make(map[string]map[slotCellKey]map[slotPositionKey]struct{})
	for _, candidate := range candidates {
		if !validUserID(candidate.UserID) || candidate.SlotID <= 0 || candidate.Weekday < 1 || candidate.Weekday > 7 || candidate.PositionID <= 0 {
			continue
		}
	...
	source := 0
	nodeCount := 1
	seatNodeIDsByUser := make(map[string][]int, len(userIDs))
	employeeNodeIDs := make(map[string]int, len(userIDs))
	groupNodeIDsByUser := make(map[string][]int, len(userIDs))
	userSlotNodeIDsByUser := make(map[string]map[slotCellKey]int, len(userIDs))
...
func prepareAutoAssignSlotPositions(
	...
	for _, slotPosition := range slotPositions {
		...
		if slotPosition.Weekday < 1 || slotPosition.Weekday > 7 {
			return nil, nil, 0, fmt.Errorf("invalid weekday for slot %d: %d", slotPosition.SlotID, slotPosition.Weekday)
		}
		if slotPosition.RequiredHeadcount <= 0 {
			return nil, nil, 0, fmt.Errorf("invalid headcount for slot %d position %d", slotPosition.SlotID, slotPosition.PositionID)
		}
...
func buildAutoAssignOverlapGroups(
	...
	slotCellsByUser map[string]map[slotCellKey]struct{},
) ([]string, map[string][][]slotCellKey) {
	userIDs := make([]string, 0, len(slotCellsByUser))
	...
	result := make(map[string][][]slotCellKey, len(userIDs))
