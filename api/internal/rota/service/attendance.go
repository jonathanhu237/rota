...
type attendanceRepository interface {
	ListLeaderCandidateShifts(ctx context.Context, publicationID int64, userID string, fromDate, toDate time.Time) ([]model.AttendanceShiftRef, error)
...
func (s *AttendanceService) RecordLeaderArrival(
	...
	if !validUserID(input.ActorUserID) ||
...
func (s *AttendanceService) AdminClearArrival(ctx context.Context, input AdminClearArrivalInput) error {
	if !validUserID(input.ActorUserID) || input.PublicationID <= 0 || input.RecordID <= 0 {
...
func (s *AttendanceService) UpdateAttendanceSettings(
	...
	if !validUserID(input.ActorUserID) || input.PublicationID <= 0 {
		return nil, ErrInvalidInput
	}
...
func (s *AttendanceService) ensureActiveUser(ctx context.Context, userID string) error {
...
func callerIsResponsible(roster []*AttendanceRosterEntry, userID string) bool {
	responsibleCount := 0
...
func findRosterEntry(roster []*AttendanceRosterEntry, assignmentID int64, userID string) *AttendanceRosterEntry {
	for _, row := range roster {
