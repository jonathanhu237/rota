...
type publicationRepository interface {
	...
	ListSubmissionSlots(ctx context.Context, publicationID int64, userID string) ([]model.SlotRef, error)
	...
	GetAdminAvailabilityUser(ctx context.Context, publicationID int64, userID string) (*model.User, []*model.Position, error)
...
func (s *PublicationService) ListAvailabilitySubmissionSlots(
	...
	if publicationID <= 0 || !validUserID(userID) {
		return nil, ErrInvalidInput
	}
...
func (s *PublicationService) CreateAvailabilitySubmission(
	...
	if input.PublicationID <= 0 || !validUserID(input.UserID) || input.SlotID <= 0 || input.Weekday < 1 || input.Weekday > 7 {
		return nil, ErrInvalidInput
	}
