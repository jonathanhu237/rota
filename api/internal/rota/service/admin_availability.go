...
func (s *PublicationService) GetAdminAvailabilityDetail(
	...
	if input.PublicationID <= 0 || !validUserID(input.UserID) {
		return nil, ErrInvalidInput
	}
...
func recordAdminAvailabilityAudit(ctx context.Context, action string, publicationID int64, userID string, slots []model.SlotRef) {
	for _, slot := range slots {
