...
func (r nullableAttendanceRecord) toModel(userName, userEmail string) *model.AttendanceRecord {
	...
	if r.RecordedByUserID.Valid {
		record.RecordedByUserID = &r.RecordedByUserID.String
	}
	if r.UpdatedByUserID.Valid {
		record.UpdatedByUserID = &r.UpdatedByUserID.String
	}
	return record
}
...
func scanAttendanceRecord(row scanner) (*model.AttendanceRecord, error) {
	...
	if recordedBy.Valid {
		record.RecordedByUserID = &recordedBy.String
	}
	if updatedBy.Valid {
		record.UpdatedByUserID = &updatedBy.String
	}
	return record, nil
}
...
func mapAttendanceWriteError(err error) error {
	...
	if ok && code == "23505" && constraint == attendanceRecordsUniqueKey {
