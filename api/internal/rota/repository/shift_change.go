...
func createShiftChangeRequest(
	...
	var counterpartUser sql.NullString
	if params.CounterpartUserID != nil {
		counterpartUser = sql.NullString{String: *params.CounterpartUserID, Valid: true}
	}
...
type ShiftChangeAudience struct {
	...
	ViewerUserID    string // non-admin: rows where viewer is requester, counterpart, or (for pool) qualified
...
func (r *ShiftChangeRepository) SetLeaveIDTx(
	...
	if ok && code == "23505" && constraint == shiftChangeRequestsActiveLeaveOccurrenceIndex {
...
func updateShiftChangeState(ctx context.Context, db dbtx, params UpdateStateParams) error {
	...
	if params.DecidedByUserID != nil {
		decidedBy = sql.NullString{String: *params.DecidedByUserID, Valid: true}
	}
...
func scanShiftChangeRequest(row scanner) (*model.ShiftChangeRequest, error) {
	var (
		...
		counterpartUser       sql.NullString
