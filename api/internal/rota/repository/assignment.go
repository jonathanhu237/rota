...
type AssignmentBoardSlotKey struct {
	SlotID  int64
	Weekday int
}
...
func (r *PublicationRepository) GetUserByID(ctx context.Context, id string) (*model.User, error) {
	const query = `
		SELECT id, email, name, is_admin, status, version, language_preference, theme_preference
		FROM users
		WHERE id = $1;
	`
...
func createAssignmentWithScheduleCheck(
	...
	if exists, err := assignmentExistsForUserSlotWeekday(
		ctx,
		tx,
...
func (r *PublicationRepository) ListAssignmentCandidates(
	...
	const query = `
		...
		WHERE asub.publication_id = $1
			AND u.status = 'active'
		ORDER BY asub.weekday ASC, asub.slot_id ASC, tsp.position_id ASC, u.id ASC;
	`
...
func LockAndCheckUserStatus(
	ctx context.Context,
	tx *sql.Tx,
	publicationID int64, userID string,
) error {
	if tx == nil {
...
func lockUserSchedule(ctx context.Context, tx *sql.Tx, publicationID int64, userID string) error {
	var locked bool
...
func scheduleAdvisoryLockKey(publicationID int64, userID string) int64 {
	hash := fnv.New64a()
	var buf [8]byte
	binary.BigEndian.PutUint64(buf[:], uint64(publicationID))
	_, _ = hash.Write(buf[:])
	_, _ = hash.Write([]byte(userID))
	return int64(hash.Sum64())
}

func getUserStatus(ctx context.Context, db dbtx, userID string) (model.UserStatus, error) {
	const query = `
		SELECT CASE WHEN disabled_at IS NULL THEN 'active' ELSE 'disabled' END
		FROM auth_users
		WHERE id = $1::uuid
		FOR UPDATE;
	`
...
func assignmentExistsForUserSlotWeekday(
	ctx context.Context,
	db dbtx,
	publicationID int64, userID string, slotID int64,
	weekday int,
) (bool, error) {
	const query = `
		SELECT 1
	...
	var one int
	err := db.QueryRowContext(ctx, query, publicationID, userID, slotID, weekday).Scan(&one)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	if err != nil {
...
func (r *PublicationRepository) GetAssignmentBoardView(
	ctx context.Context,
	publicationID int64,
) (map[AssignmentBoardSlotKey]*AssignmentBoardSlotView, error) {
	const query = `
		SELECT
			ts.id,
			ts.template_id,
	...
	board := make(map[AssignmentBoardSlotKey]*AssignmentBoardSlotView)
	for rows.Next() {
		var (
			slotID            int64
			templateID        int64
		...
		key := AssignmentBoardSlotKey{SlotID: slotID, Weekday: weekday}
		slotView := board[key]
		if slotView == nil {
...
func (r *PublicationRepository) ListAssignmentBoardEmployees(
	...
	employeeByID := make(map[string]*model.AssignmentBoardEmployee)
	for rows.Next() {
		var (
			userID     string
			name       string
	...
	userIDs := make([]string, 0, len(employees))
...
func getAssignmentByKey(
	...
	const query = `
		SELECT
			...
			a.created_at
		FROM assignments a
		WHERE a.publication_id = $1 AND a.user_id = $2 AND a.slot_id = $3 AND a.weekday = $4;
	`

	return scanAssignment(db.QueryRowContext(ctx, query, publicationID, userID, slotID, weekday))
}
...
func mapAssignmentWriteError(err error) error {
	...
	code, constraint, ok := postgresError(err)
	if ok && code == "23505" &&
