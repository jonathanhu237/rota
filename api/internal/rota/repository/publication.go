...
func (r *PublicationRepository) IsUserQualifiedForPosition(
	ctx context.Context,
	userID string, positionID int64,
) (bool, error) {
	const query = `
		SELECT EXISTS (
			SELECT 1
...
func mapPublicationWriteError(err error) error {
	...
	switch {
	case code == "23505" && constraint == publicationsSingleNonEndedIndex:
		return ErrPublicationAlreadyExists
	case code == "23514" && constraint == publicationsWindowCheckName:
		return ErrInvalidPublicationWindow
	case code == "23514" && constraint == publicationsOvertimeWindowHoursCheck:
		return ErrInvalidPublicationWindow
	default:
		return err
	}
