...
func mailTaskBranding(ctx context.Context, tx *sql.Tx) (string, string, error) {
	var name, organizationName string
	if err := tx.QueryRowContext(ctx, `SELECT COALESCE(system_name, 'Temvia'), COALESCE(organization_name, '') FROM auth_system_identity WHERE singleton = true`).Scan(&name, &organizationName); err != nil {
		if err == sql.ErrNoRows {
			return application.DefaultSystemName, application.DefaultOrganizationName, nil
		}
		return "", "", err
	}
	if strings.TrimSpace(name) == "" {
		name = application.DefaultSystemName
	}
	return name, strings.TrimSpace(organizationName), nil
}

func mailTaskSystemName(ctx context.Context, tx *sql.Tx) (string, error) {
	name, _, err := mailTaskBranding(ctx, tx)
	return name, err
}
...
func (s *Store) EnqueueMailTask(ctx context.Context, input application.MailTaskInput) (application.MailTask, error) {
	...
	if input.Message != nil {
		...
		if message.OrganizationName == "" {
			message.OrganizationName = organizationName
		}
	...
	err = s.db.QueryRowContext(ctx, `
		INSERT INTO auth_mail_outbox (
			...
			locale, system_name, organization_name, material_ciphertext, submitted_by,
			round_number, round_attempt_count, expires_at, created_at
		)
		VALUES (
			$1, NULLIF($2, '')::uuid, NULLIF($3, '')::uuid, $4, $5,
			NULLIF($6, ''), NULLIF($7, '')::uuid, $8, $9, $10, $11, $12,
			NULLIF($13, '')::uuid, 1, 0, $14, $15
		)
		RETURNING id::text`,
		...
		string(locale), systemName, organizationName, nullableBytes(materialCiphertext), input.SubmittedBy, expiresAt, createdAt).Scan(&id)
	if err != nil {
		return application.MailTask{}, err
	}
