...
func (s *Store) ClaimMail(ctx context.Context, leaseToken string, leaseDuration time.Duration) (*application.MailJob, error) {
	...
	var recipientName, recipientEmail, systemName, organizationName string
	...
	err = tx.QueryRowContext(ctx, `
		SELECT o.id::text, o.kind, o.user_id::text, o.invitation_id::text,
		       ...
		       COALESCE(o.system_name, 'Temvia'), COALESCE(o.organization_name, ''), o.locale,
		...
		LIMIT 1`,
	).Scan(&job.ID, &kind, &userID, &invitationID, &recipientName, &recipientEmail, &systemName, &organizationName, &locale,
	...
	job.OrganizationName = organizationName
