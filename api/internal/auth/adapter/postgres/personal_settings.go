...
func (s *Store) ChangePassword(ctx context.Context, userID string, expectedAuthVersion int64, passwordHash string, locale domain.Locale, notificationTTL time.Duration) (domain.User, time.Time, error) {
	...
	systemName, organizationName, err := mailTaskBranding(operationCtx, tx)
	if err != nil {
		return domain.User{}, time.Time{}, err
	}
	expiresAt := changedAt.Add(notificationTTL)
	material, err := s.sealMailTaskMaterial(application.MailTaskMaterial{Version: 1, Kind: application.MailPasswordChanged, Name: user.Name, Email: user.Email, Locale: locale, SystemName: systemName, OrganizationName: organizationName, CreatedAt: changedAt, ExpiresAt: expiresAt})
	if err != nil {
		return domain.User{}, time.Time{}, err
	}
	if _, err := tx.ExecContext(operationCtx, `
		...
		VALUES ('password_changed', $1::uuid, $2, $3, $4, $5, $6, $7, $8, $9)`, userID, user.Email, user.Name, locale, systemName, organizationName, nullableBytes(material), expiresAt, changedAt); err != nil {
		return domain.User{}, time.Time{}, err
	}
...
func (s *Store) RequestEmailChange(ctx context.Context, userID string, newEmail domain.Email, selector, digest []byte, ttl time.Duration, locale domain.Locale) (domain.EmailChangeRequest, error) {
	...
	material, err := s.sealMailTaskMaterial(application.MailTaskMaterial{Version: 1, Kind: application.MailEmailChangeCode, Name: currentName, Email: newEmail.Display, Locale: locale, SystemName: systemName, OrganizationName: organizationName, CreatedAt: taskCreatedAt, ExpiresAt: request.ExpiresAt, EmailSelector: selector, VerifierDigest: digest})
	if err != nil {
		return domain.EmailChangeRequest{}, err
	}
	if _, err := tx.ExecContext(operationCtx, `
		INSERT INTO auth_mail_outbox (kind, user_id, recipient_email, recipient_name, email_change_request_id, email_change_selector, locale, system_name, organization_name, material_ciphertext, expires_at, created_at)
		VALUES ('email_change_code', $1::uuid, $2, $3, $4::uuid, $5, $6, $7, $8, $9, $10, $11)`, userID, newEmail.Display, currentName, request.ID, selector, locale, systemName, organizationName, nullableBytes(material), request.ExpiresAt, taskCreatedAt); err != nil {
		return domain.EmailChangeRequest{}, err
	}
	if err := tx.Commit(); err != nil {
		return domain.EmailChangeRequest{}, err
...
func (s *Store) ResendEmailChange(ctx context.Context, userID string, selector, digest []byte, ttl time.Duration, locale domain.Locale) (domain.EmailChangeRequest, error) {
	...
	material, err := s.sealMailTaskMaterial(application.MailTaskMaterial{Version: 1, Kind: application.MailEmailChangeCode, Name: currentName, Email: request.NewEmail, Locale: locale, SystemName: systemName, OrganizationName: organizationName, CreatedAt: taskCreatedAt, ExpiresAt: request.ExpiresAt, EmailSelector: selector, VerifierDigest: digest})
	if err != nil {
		return domain.EmailChangeRequest{}, err
	}
	if _, err := tx.ExecContext(operationCtx, `
		INSERT INTO auth_mail_outbox (kind, user_id, recipient_email, recipient_name, email_change_request_id, email_change_selector, locale, system_name, organization_name, material_ciphertext, expires_at, created_at)
		VALUES ('email_change_code', $1::uuid, $2, $3, $4::uuid, $5, $6, $7, $8, $9, $10, $11)`, userID, request.NewEmail, currentName, request.ID, selector, locale, systemName, organizationName, nullableBytes(material), request.ExpiresAt, taskCreatedAt); err != nil {
		return domain.EmailChangeRequest{}, err
	}
	if err := tx.Commit(); err != nil {
		return domain.EmailChangeRequest{}, err
...
func (s *Store) CompleteEmailChange(ctx context.Context, userID, requestID string, presentedDigest []byte, locale domain.Locale, notificationTTL time.Duration) (domain.User, time.Time, error) {
	...
	expiresAt := changedAt.Add(notificationTTL)
	material, err := s.sealMailTaskMaterial(application.MailTaskMaterial{Version: 1, Kind: application.MailEmailChanged, Name: user.Name, Email: request.OldEmail, Locale: locale, SystemName: systemName, OrganizationName: organizationName, CreatedAt: changedAt, ExpiresAt: expiresAt})
	if err != nil {
		return domain.User{}, time.Time{}, err
	}
	if _, err := tx.ExecContext(operationCtx, `
		INSERT INTO auth_mail_outbox (kind, user_id, recipient_email, recipient_name, locale, system_name, organization_name, material_ciphertext, expires_at, created_at)
		VALUES ('email_changed', $1::uuid, $2, $3, $4, $5, $6, $7, $8, $9)`, userID, request.OldEmail, user.Name, locale, systemName, organizationName, nullableBytes(material), expiresAt, changedAt); err != nil {
		return domain.User{}, time.Time{}, err
	}
	if err := tx.Commit(); err != nil {
		return domain.User{}, time.Time{}, err
