...
import (
	...
	postgresadapter "github.com/jonathanhu237/rota/api/internal/auth/adapter/postgres"
...
func TestUserLifecycleMigrationPreservesExistingRecordsIntegration(t *testing.T) {
	...
	// The current schema also contains the Rota compatibility view named users;
	// remove that newer dependent object before directly exercising the legacy
	// lifecycle migration. Normal production rollback removes migration 13 first.
	if _, err := db.ExecContext(ctx, `DROP VIEW IF EXISTS users`); err != nil {
		t.Fatal(err)
	}
