...
func identityMultipartRequestWithValues(t *testing.T, method, path string, revision int64, action, systemName, organizationName, fileName, mediaType string, icon []byte) *http.Request {
	return identityMultipartRequestWithOrganization(t, method, path, revision, action, systemName, organizationName, fileName, mediaType, icon)
}

func identityMultipartRequestWithOrganization(t *testing.T, method, path string, revision int64, action, systemName, organizationName, fileName, mediaType string, icon []byte) *http.Request {
	t.Helper()
	var body bytes.Buffer
	writer := multipart.NewWriter(&body)
	_ = writer.WriteField("systemName", systemName)
	_ = writer.WriteField("organizationName", organizationName)
...
func TestSystemIdentityHTTPAllowsReadOnlyReadButRejectsWrite(t *testing.T) {
	store := &identityHTTPStoreFake{configured: true, record: application.SystemIdentityRecord{SystemName: "星河管理", OrganizationName: "星河科技", Revision: 4}}
	...
	if read.Code != http.StatusOK || !bytes.Contains(read.Body.Bytes(), []byte(`"systemName":"星河管理"`)) || !bytes.Contains(read.Body.Bytes(), []byte(`"organizationName":"星河科技"`)) {
...
func TestSystemIdentityHTTPAcceptsAndReturnsOrganizationBranding(t *testing.T) {
	store := &identityHTTPStoreFake{configured: true, record: application.SystemIdentityRecord{SystemName: "Rota", Revision: 1}}
	...
	request := identityMultipartRequestWithOrganization(t, http.MethodPut, "/api/settings/system-identity", 1, application.SystemIconPreserve, "Rota", "Acme Operations", "icon.png", "image/png", nil)
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || store.record.OrganizationName != "Acme Operations" || !bytes.Contains(response.Body.Bytes(), []byte(`"organizationName":"Acme Operations"`)) {
		t.Fatalf("organization branding save = %d %s record=%#v", response.Code, response.Body.String(), store.record)
	}
}
