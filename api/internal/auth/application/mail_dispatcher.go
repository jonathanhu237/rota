...
func (d *MailDispatcher) composePersistedMaterial(job MailJob) (OutgoingMail, error) {
	...
	if material.Message != nil {
		...
		return withOrganizationBranding(message), nil
	}
	stableJob := job
	...
	stableJob.OrganizationName = material.OrganizationName
...
func (d *MailDispatcher) composeLegacy(ctx context.Context, job MailJob) (OutgoingMail, error) {
	...
	job.SystemName = identity.SystemName
	job.OrganizationName = identity.OrganizationName
...
func (d *MailDispatcher) composeSnapshot(job MailJob) (OutgoingMail, error) {
	...
	if job.Kind == MailPasswordReset {
		...
		return withOrganizationBranding(resetMail(message, link, job.Locale, job.ExpiresAt)), nil
	}
	if job.Kind == MailPasswordChanged {
		return withOrganizationBranding(changedMail(message, job.CreatedAt, job.Locale)), nil
	}
	if job.Kind == MailEmailChanged {
		return withOrganizationBranding(emailChangedMail(message, job.CreatedAt, job.Locale)), nil
	}
	if job.Kind == MailEmailChangeCode {
		...
		return withOrganizationBranding(emailChangeCodeMail(message, code, job.Locale, job.ExpiresAt)), nil
	}
	if job.Kind == MailUserInvitation {
		...
		return withOrganizationBranding(invitationMail(message, link, job.Locale, job.ExpiresAt)), nil
	}
...
func withOrganizationBranding(message OutgoingMail) OutgoingMail {
	organizationName := strings.TrimSpace(message.OrganizationName)
	if organizationName == "" {
		return message
	}
	message.Text = strings.TrimSpace(message.Text) + "\n\n" + organizationName
	if strings.TrimSpace(message.HTML) != "" {
		footer := `<p style="margin:24px 0 0;color:#98A2B3;font-family:-apple-system,BlinkMacSystemFont,&quot;Segoe UI&quot;,Roboto,Helvetica,Arial,sans-serif;font-size:12px;line-height:18px;">` + html.EscapeString(organizationName) + `</p>`
		if strings.Contains(message.HTML, "</body></html>") {
			message.HTML = strings.Replace(message.HTML, "</body></html>", footer+"</body></html>", 1)
		} else {
			message.HTML += footer
		}
	}
