...
func (s *SettingsManagement) TestEmailSettingsTask(ctx context.Context, actorID string, input EmailSettingsInput, recipient string) (MailTask, error) {
	...
	message := testMail(OutgoingMail{Kind: MailTest, SystemName: systemName, OrganizationName: organizationName, Name: "administrator", To: recipient, Locale: locale})
	...
	task, enqueueErr := s.mailTasks.EnqueueMailTask(ctx, MailTaskInput{Kind: MailTest, RecipientEmail: recipient, RecipientName: "administrator", Locale: locale, SystemName: systemName, OrganizationName: organizationName, ExpiresAt: expires, SubmittedBy: actorID, Message: &message})
	if enqueueErr != nil {
