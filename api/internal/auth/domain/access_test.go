...
func TestEmployeeRoleHasSelfServiceWithoutAdministrativeGrants(t *testing.T) {
	catalog := DefaultPermissionCatalog()
	grants, err := catalog.ValidateFeatureSet([]PermissionKey{"rota.self"})
	if err != nil {
		t.Fatalf("employee self-service role rejected: %v", err)
	}
	if !reflect.DeepEqual(grants, []PermissionKey{"rota.self"}) {
		t.Fatalf("employee role gained extra authority: %v", grants)
	}
	if _, err := catalog.ValidateFeatureSet(nil); err == nil {
		t.Fatal("empty role must still be rejected")
	}
}

func TestPermissionCatalogKeepsReadWriteIndependentAndValidatesCombinations(t *testing.T) {
	catalog := DefaultPermissionCatalog()
	for _, key := range []PermissionKey{PermissionRotaRead, PermissionRotaManage} {
		if !catalog.Has(key) {
			t.Fatalf("default catalog is missing %q", key)
		}
	}
	if _, err := catalog.ValidateFeatureSet([]PermissionKey{PermissionRotaRead}); err != nil {
		t.Fatalf("Rota read grant rejected: %v", err)
	}
	if _, err := catalog.ValidateFeatureSet([]PermissionKey{PermissionRotaManage}); err != nil {
		t.Fatalf("Rota manage grant rejected: %v", err)
	}
