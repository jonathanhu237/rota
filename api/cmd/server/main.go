...
import (
	...
	"github.com/jonathanhu237/rota/api/internal/auth/adapter/httpapi"
	mailadapter "github.com/jonathanhu237/rota/api/internal/auth/adapter/mail"
	"github.com/jonathanhu237/rota/api/internal/auth/adapter/password"
	"github.com/jonathanhu237/rota/api/internal/auth/adapter/postgres"
	"github.com/jonathanhu237/rota/api/internal/auth/application"
	"github.com/jonathanhu237/rota/api/internal/auth/domain"
	"github.com/jonathanhu237/rota/api/internal/config"
	rotahttp "github.com/jonathanhu237/rota/api/internal/rota/httpapi"
	rotarepo "github.com/jonathanhu237/rota/api/internal/rota/repository"
	rotaservice "github.com/jonathanhu237/rota/api/internal/rota/service"
)
...
func buildRotaHandler(db *sql.DB, cfg config.Config, auth *application.Authentication, mailTaskSecretBox application.SecretBox) http.Handler {
	positionRepo := rotarepo.NewPositionRepository(db)
	templateRepo := rotarepo.NewTemplateRepository(db)
	publicationRepo := rotarepo.NewPublicationRepository(db)
	shiftChangeRepo := rotarepo.NewShiftChangeRepository(db)
	leaveRepo := rotarepo.NewLeaveRepository(db)
	attendanceRepo := rotarepo.NewAttendanceRepository(db)
	userPositionRepo := rotarepo.NewUserPositionRepository(db)
	brandingRepo := rotarepo.NewBrandingRepository(db)
	outboxRepo := rotarepo.NewOutboxRepository(db, mailTaskSecretBox)
	logger := slog.Default()

	positionService := rotaservice.NewPositionService(positionRepo)
	templateService := rotaservice.NewTemplateService(templateRepo, positionRepo)
	publicationService := rotaservice.NewPublicationService(
		publicationRepo,
		nil,
		rotaservice.WithPublicationShiftChangeNotifications(shiftChangeRepo, outboxRepo, cfg.PublicURL, logger),
		rotaservice.WithPublicationBrandingProvider(brandingRepo),
	)
	shiftChangeService := rotaservice.NewShiftChangeService(
		shiftChangeRepo,
		publicationRepo,
		outboxRepo,
		cfg.PublicURL,
		nil,
		logger,
		rotaservice.WithShiftChangeBrandingProvider(brandingRepo),
	)
	leaveService := rotaservice.NewLeaveService(leaveRepo, shiftChangeRepo, shiftChangeService, publicationRepo, nil)
	attendanceService := rotaservice.NewAttendanceService(attendanceRepo, publicationRepo, nil)
	userPositionService := rotaservice.NewUserPositionService(userPositionRepo)
	auditRecorder := rotarepo.NewAuditRecorder(db, logger)

	return rotahttp.NewRouter(auth, cfg.CookieName, auditRecorder, rotahttp.RotaHandlers{
		Positions:     rotahttp.NewPositionHandler(positionService),
		Templates:     rotahttp.NewTemplateHandler(templateService),
		Publications:  rotahttp.NewPublicationHandler(publicationService),
		Attendance:    rotahttp.NewAttendanceHandler(attendanceService),
		UserPositions: rotahttp.NewUserPositionHandler(userPositionService),
		ShiftChanges:  rotahttp.NewShiftChangeHandler(shiftChangeService),
		Leaves:        rotahttp.NewLeaveHandler(leaveService),
	}, rotahttp.WithOrigin(cfg.Origin), rotahttp.WithTrustedProxyCIDRs(cfg.TrustedProxyCIDRs))
}

func mountRotaHandler(base, rota http.Handler) http.Handler {
	mux := http.NewServeMux()
	mux.Handle("/api/rota/", http.StripPrefix("/api/rota", rota))
	mux.Handle("/api/rota", http.StripPrefix("/api/rota", rota))
	mux.Handle("/", base)
	return mux
}
...
func run() int {
	...
	authService := application.NewAuthentication(postgresStore, hasher, postgresStore, postgresStore, random, domain.DefaultPermissionCatalog())
	...
	handler = mountRotaHandler(handler, buildRotaHandler(db, cfg, authService, mailTaskSecretBox))
	server := &http.Server{
		Addr:              cfg.HTTPAddr,
		Handler:           handler,
